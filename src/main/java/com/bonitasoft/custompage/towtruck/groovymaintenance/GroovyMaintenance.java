...
public class GroovyMaintenance {
    ...
    private static final String CSTJSON_DIRECTRESTAPI = "directRestApi";
    private static final String CSTJSON_LISTEVENTS = "listevents";
    private static final String CSTJSON_STATUS = "status";

    public static Logger logger = Logger.getLogger(GroovyMaintenance.class.getName());
    ...
    public static Map<String, Object> getGroovyIntepretation(HttpServletRequest request, String groovySource, File pageDirectory) {
        ...
        resultInterpretation.result.put(CSTJSON_LISTEVENTS, BEventFactory.getHtml(resultInterpretation.listEvents));
    ...
    public static Map<String, Object> getGroovyMaintenance(HttpServletRequest request, String groovyCode, File pageDirectory) {
        ...
        try {
            ...
            if (groovyArtefact == null) {
                ...
                // load it
                BonitaStoreResult storeResult = groovyArtefact.getStore().loadArtifact(groovyArtefact, UrlToDownload.URLDOWNLOAD, logBox);
                ...
                if (!BEventFactory.isError(storeResult.getEvents())) {
                    ...
                    result.put(CSTJSON_DIRECTRESTAPI, getDirectRestApi(groovyCode, resultInterpretation.mapPlaceHolder));
                    listEvents.addAll(resultInterpretation.listEvents);
                }

            }
        } catch (Exception e) {
            result.put(CSTJSON_STATUS, "FAILED");
    ...
    public static class ResultInterpretation {
        ...
        Map<String, Object> result = new HashMap<>();
        List<BEvent> listEvents = new ArrayList<>();
        Map<String, AttributeHolder> mapPlaceHolder = new HashMap<>();
        List<Map<String, Object>> listPlaceHolder = new ArrayList<>();
    ...
    private static ResultInterpretation interpretation(String groovySource, HttpServletRequest request, String groovyCode) {
        ...
        if (!BEventFactory.isError(resultInterpretation.listEvents)) {
            resultInterpretation.result.put(CSTJSON_STATUS, "DOWNLOADED");
            resultInterpretation.listEvents.add(GROOVY_DOWNLOADED);
        } else
            resultInterpretation.result.put(CSTJSON_STATUS, "NOTEXIST");
    ...
    public static Map<String, Object> executeGroovyMaintenance(HttpServletRequest request, String groovySource, List<Map<String, String>> valuePlaceHolders, Binding binding) {
        ...
        if (groovySource == null) {
            ...
            result.put(CSTJSON_DIRECTRESTAPI, getDirectRestApi(groovyCode, mapPlaceHolder));
    ...
    private static String getDirectRestApi(String groovyCode, Map<String, AttributeHolder> mapPlaceHolder) {
        ...
        String directRestApi = "/bonita/portal/custom-page/custompage_towtruck/?page=custompage_towtruck&action=groovyrest&code=" + groovyCode;
        for (AttributeHolder attribut : mapPlaceHolder.values()) {
